...
class GenerationSample:
    ...
    def generate_next_action(self) -> ActionSpace:
        ...
        pprint(prompt)
        gpt_response = GenerationSample._gpt_call(
    ...
    def as_oai_request_object(self):
        return {
            "custom_id": f"{self.experiment_id}-{self.sample_idx}",
            "method": "POST",
...
def generate_samples(
    ...
    for pmt, ss, acs, lc in itertools.product(
        ...
        for state in ss.get_sweep(ss.get_sweep_values()):
            ...
            sample_idx += 1
    return random.sample(generation_samples, limit)


def run_simulations(
    client: OpenAI, model: str = "gpt-3.5-turbo", batch_requests: bool = False
):
    ...
    if batch_requests:
        generation_requests = [
            sample.as_oai_request_object() for sample in generation_samples
        ]
        ...
        for sample in generation_samples:
            next_action = sample.generate_next_action()
